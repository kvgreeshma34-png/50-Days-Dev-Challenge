...
function initGitHubLookup() {
    ...
    async function fetchContributor(username) {
        ...
        profileContainer.innerHTML = `<p style="text-align: center; color: #64748b;">Fetching data from GitHub...</p>`;

        try {
            const response = await fetch(`https://api.github.com/users/${cleanUsername}`);
            if (!response.ok) throw new Error("User not found. Please check the username.");
            ...
            `;
            showToast("GitHub profile fetched successfully!", "success");
        } catch (error) {
    ...
    if (!searchBtn.dataset.lookupInitialized) {
        searchBtn.dataset.lookupInitialized = 'true';
        searchBtn.addEventListener('click', () => fetchContributor(usernameInput.value));
...
function showToast(message, type = 'success') {
    ...
    setTimeout(() => {
        ...
        toast.style.transform = 'translateY(20px)'; // Fixed syntax error here
        setTimeout(() => {
            toast.remove();
        }, 300);
    }, 3000);
