// ==========================================
// MAIN APPLICATION ORCHESTRATOR (DAY 38)
// ==========================================

import { showToast, debounce } from './utils.js';
import { fetchGitHubUser, fetchPaginatedPosts, deleteRecord, updateRecord, fetchDashboardData } from './api.js';
...
function initGitHubLookup() {
    ...
    async function handleContributorSearch(username, updateHistory = true) {
        ...
        if (!cleanUsername) {
            ...
            if (updateHistory) {
                updateURLParameter('user', null);
            }
            return;
        }

        if (updateHistory) {
            updateURLParameter('user', cleanUsername);
        }

        if (searchInput.value !== cleanUsername) {
    ...
    if (!searchInput.dataset.lookupInitialized) {
        ...
        const debouncedSearch = debounce((e) => {
            handleContributorSearch(e.target.value, true);
        }, 500);
    ...
    if (userFromURL) {
        handleContributorSearch(userFromURL, false);
    }
}

// --- DAY 37: PARALLEL DASHBOARD INITIALIZER ---
function initDashboardView() {
    ...
    dashboardBtn.addEventListener('click', async () => {
        ...
        if (!username) {
            showToast("Please enter a username for the dashboard.", "error");
            return;
        }
        ...
        try {
            ...
            `;
            showToast("Dashboard data loaded successfully via parallel requests!", "success");
...
// --- DAY 38: WEBSOCKET TERMINAL BINDINGS ---
...
function initMentorModal() {
    ...
    // Scoped strictly to explicit mentor cards to prevent overlapping button triggers
...
function initApp() {
    console.log("Synexus Core ES6 Modular Architecture Initialized (Day 38 WebSockets Active).");
    initThemeToggle();
    initMobileMenu();
    ...
    initGitHubLookup();
    initDashboardView();
    initWebSocketTerminal(); // Day 38 addition
    initMentorModal();
    initFormValidation();
